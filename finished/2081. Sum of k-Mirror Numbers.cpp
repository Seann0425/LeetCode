...
private:
    static constexpr long long ans[][30] = {
        ...
        {1,          2,          3,          4,          6,           88,
         252,        282,        626,        676,        1221,        15751,
         18881,      10088001,   10400401,   27711772,   30322303,    47633674,
         65977956,   808656808,  831333138,  831868138,  836131638,   836181638,
        ...
        {1,         2,           3,           4,           5,           6,
         8,         121,         171,         242,         292,         16561,
         65656,     2137312,     4602064,     6597956,     6958596,     9470749,
         61255216,  230474032,   466828664,   485494584,   638828836,   657494756,
...
public:
    long long kMirror(int k, int n) { return accumulate(ans[k - 2], ans[k - 2] + n, 0LL); }
};
...
public:
    long long kMirror(int k, int n) {
        auto is_kpalindrome = [&](long long x) -> bool {
            auto len = 0;
            while (x) {
                digit[len++] = x % k;
                x /= k;
            }
            for (auto i = 0, j = len - 1; i < j; ++i, --j) {
        ...
        while (count < n) {
            auto right = left * 10;
            for (auto is_even = 0; is_even < 2; ++is_even) {
                // i is the half of the palindrome number in base-10
                for (auto i = left; i < right && count < n; ++i) {
                    auto palindrome = static_cast<long long>(i);
                    auto x = is_even ? i : i / 10;
                    while (x) {
                        palindrome = palindrome * 10 + x % 10;
                        x /= 10;
                    }
                    if (is_kpalindrome(palindrome)) {
                        ++count;
                        ans += palindrome;
                    }
                }
