...
public:
    int minimumDeletions(vector<int> &nums) {
        ...
        auto minimum = static_cast<size_t>(distance(nums.begin(), min_it));
        auto maximum = static_cast<size_t>(distance(nums.begin(), max_it));
        if (minimum > maximum) swap(minimum, maximum);
        return static_cast<int>(min({maximum + 1, n - minimum, n - maximum + minimum + 1}));
    }
};